...
OUTPUT_FILE = "scraped_gem5_docs_with_html.json" # New output file name
...
def scrape_page_content(url):
    """
    Scrape the title and main HTML content from a single documentation page.
    """
    try:
    ...
    # Instead of getting just the text, we get the entire HTML content of the div
    # We convert the BeautifulSoup tag object back to a string to store it.
    html_content = str(content_div)
    
    return {
        "url": url,
        "title": title,
        "html_content": html_content # Store HTML content, not plain text
    }
